// var categorys = null, addresses = null, carts = null, orders = null, orderDetails = null, user = null;
// if(Cookies.get('category')) {
// 	categorys = JSON.parse(Cookies.get('category'));
// } else {
// 	categorys = category;
// }
// if(Cookies.get('address')) {
// 	addresses = JSON.parse(Cookies.get('address'));
// } else {
// 	addresses = address;
// }
// if(Cookies.get('cart')) {
// 	carts = JSON.parse(Cookies.get('cart'));
// } else {
// 	carts = cart;
// }
// if(Cookies.get('order')) {
// 	orders = JSON.parse(Cookies.get('order'));
// } else {
// 	orders = order;
// }
// if(Cookies.get('orderDetail')) {
// 	orderDetails = JSON.parse(Cookies.get('orderDetail'));
// } else {
// 	orderDetails = orderDetail;
// }
// if(Cookies.get('users')) {
// 	users = JSON.parse(Cookies.get('user'));
// } else {
// 	users = user;
// }
// 向cookie中写入一个键init表示访问过主页
Cookies.set('init', 'true');
// 主页，把网站中所的数据放在cookie中
if(!Cookies.get('user'))   Cookies.set('user', JSON.stringify(user));
if(!Cookies.get('category'))   Cookies.set('category', JSON.stringify(category));
if(!Cookies.get('address'))  Cookies.set('address', JSON.stringify(address));
if(!Cookies.get('cart'))  Cookies.set('cart', JSON.stringify(cart));
if(!Cookies.get('order'))  Cookies.set('order', JSON.stringify(order));
if(!Cookies.get('orderDetail'))  Cookies.set('orderDetail', JSON.stringify(orderDetail));
...
// part4
var timers = null;
function banner2Toggle(index) {
	document.querySelector('.banner2-btn>li.show').className = '';
	banner2btn[index].className = 'show';
	document.querySelector('ul.banner2-img').style.marginLeft = '-' + index + '00%';
};
// 按钮
var banner2btn = document.querySelectorAll('.banner2-btn>li');
for(var i = 0; i < banner2btn.length; i++) {
	banner2btn[i].index = i;
	banner2btn[i].onmouseover = function() {
		if(this.className === 'show') return;
		banner2Toggle(this.index);
	}
};
function autoPlay2() {
	timers = setInterval(function() {
		var index = document.querySelector('.banner2-btn>li.show').index;
		index = index === banner2btn.length - 1 ? 0 : index + 1;
		banner2Toggle(index);
	}, 3000);
	
};
autoPlay2();
document.querySelector('.banner2-img').onmouseover = function() {
	clearInterval(timers); 
}
document.querySelector('.banner2-img').onmouseout = function() {
	autoPlay2();
};
